    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-start justify-between">
        <label className="text-sm text-text fomt-medium">
          ...
          <p className="text-xs leading-[150%] mt-1 text-secondary-foreground">Choose an end date to stop this reward automatically.</p>
        </label>
        <Switch
          checked={enabled}
      ...
      {enabled && (
        <Popover open={isOpen} onOpenChange={setIsOpen} modal={false}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className={cn("justify-start border border-border-secondary rounded-lg text-base leading-[140%] text-text-muted bg-white h-fit py-[9px] px-3 text-left font-normal", isOpen ? "border-primary border-2" : "border-border-secondary border text-base ")}
            >
              <CalendarIcon className="mr-2 size-5 text-[#4A4A4A]" />
              <span className={cn("text-text-muted", endDate ? "text-text" : "text-text-muted")}>{endDate ? format(endDate, "PPP") : "Select end date"}</span>
            </Button>
          </PopoverTrigger>
